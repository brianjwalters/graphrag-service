#!/usr/bin/env python3
"""
Fix the embeddings table with HNSW index for 2048 dimensions
"""
import os
import sys
import asyncio
...
async def fix_embeddings_table_hnsw():
    """Fix the embeddings table with HNSW index for 2048 dimensions."""
    ...
    print("🔧 FIXING EMBEDDINGS TABLE WITH HNSW INDEX")
    print("=" * 60)
    
    client = SupabaseClient(service_name="embeddings-hnsw-fixer", use_service_role=True)
    
    # Drop and recreate the embeddings table with HNSW index (supports >2000 dims)
    fix_sql = '''
...
-- Create HNSW vector index for similarity search (supports 2048+ dimensions)
CREATE INDEX idx_graph_embeddings_vector_hnsw ON graph.embeddings 
...
SELECT 'Embeddings table fixed with HNSW index' as status;
'''
    
    print("Fixing graph.embeddings table with HNSW index...")
    try:
        ...
        if check_response.data:
            print(f"  ✅ Vector column info: {check_response.data}")
...
if __name__ == "__main__":
    try:
        success = asyncio.run(fix_embeddings_table_hnsw())
        print(f"\n{'✅ SUCCESS' if success else '❌ FAILED'}: Embeddings table fixed with HNSW")
